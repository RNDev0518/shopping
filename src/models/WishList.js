...
export class WishList extends WishSellingList {

  static ReferenceParse = Parse.Object.extend('wish_lists');

  constructor(wishList?: null){
    super(wishList, WishList.ReferenceParse);
    if (!wishList) return;
    if (wishList.id) {
      this.prepareData();
    } else {
  ...
  async save() {
    ...
    if (this.isOnline) {
      ...
      await RealmService.saveWishList(this);
      if (this.id) {
        const query = new Parse.Query(WishList.ReferenceParse);
  ...
  static async getAll() {
    ...
    return results.map(item => new WishList(item));
  }
  ...
  static async getByTags(tags: [string]) {
    ...
    const items = results.map(item => new WishList(item));
    return items;
  }
